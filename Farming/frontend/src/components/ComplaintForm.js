...
class ComplaintForm extends Component {
    ...
    componentDidMount() {
        ...
        FarmerService.getAllFarmer().then(response => {
            farmer = response.data.filter(f => f.farmerEmailId === this.state.farmerEmail);
            this.setState({
                farmerName: farmer.map(f => f.farmerName),
            ...
            <div style={{ backgroundImage: 'linear-gradient(to right, black, lightgreen)', }}>
                <div style={{
                    ...
                    <Jumbotron style={{ width: 600, backgroundColor: 'rgba(15,15,15,0.4)', filter: 'blur(10)', color: 'white' }}>
                        <h1 style={{ fontFamily: "Forte" }}>Submit Your Complaint</h1>
                        {error}
                        <br />
